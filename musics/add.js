...
module.exports = {
    ...
    async music(message, serverQueue, looping, queue, pool) {
        ...
        if(!args[1]) return message.channel.send(`You didn't provide any link/keywords! Usage: \`${message.client.prefix}${this.name} ${this.usage}\``)

        const checkURL = message.attachments.size > 0 || validURL(args[1]);

        if (checkURL) {
            if (!validYTURL(args[1])) {
                if (!validSPURL(args[1])) {
                    if (!validSCURL(args[1])) {
