  ...
  music(message, serverQueue, queue) {
    ...
    if (!serverQueue.looping) {
      ...
      if (serverQueue.repeating) {
        ...
        altGetData("UPDATE servers SET looping = 1, repeating = NULL WHERE id = '" + message.guild.id + "'", function (err) {
      ...
      serverQueue.looping = false;
        altGetData("UPDATE servers SET looping = NULL WHERE id = '" + message.guild.id + "'", function (err) {
          if (err) return message.reply("there was an error trying to update the status!");
