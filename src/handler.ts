...
import { Collection, ChatInputCommandInteraction, Guild, GuildMember, GuildMemberRoleManager, Invite, Message, ActionRowBuilder, AttachmentBuilder, ButtonBuilder, MessageComponentInteraction, EmbedBuilder, MessageReaction, SelectMenuBuilder, ModalBuilder, ModalSubmitInteraction, PartialGuildMember, PartialMessage, PartialMessageReaction, PartialUser, Role, Snowflake, TextChannel, TextInputBuilder, User, VoiceState, BaseInteraction, InteractionType, ActivityType, AuditLogEvent, ButtonStyle, TextInputStyle } from "discord.js";
import { endGiveaway } from "./commands/miscellaneous/giveaway.js";
import { emojis as pollEmojis, endPoll, updatePoll } from "./commands/miscellaneous/poll.js";
import { getRandomNumber, setTimeout_, profile, updateGuildMemberMC, nameToUuid, color, fixGuildRecord, query, duration, checkTradeW1nd, roundTo, getFont, replaceWithObj, mysqlEscape, wait, updateTokens, getTokensAndMultiplier, changeTokens } from "./function.js";
import { NorthClient, LevelData, NorthMessage, RoleMessage, NorthInteraction, GuildTimer, GuildConfig, IPrefix, ISlash } from "./classes/NorthClient.js";
...
export class Handler {
    ...
    async interactionCreate(interaction: BaseInteraction) {
        if (interaction.type === InteractionType.ApplicationCommand) return await this.commandInteraction(<ChatInputCommandInteraction>interaction);
        if (interaction.type === InteractionType.MessageComponent) return await this.messageComponentInteraction(<MessageComponentInteraction>interaction);
    }

    async commandInteraction(interaction: ChatInputCommandInteraction) {
    ...
    async messageComponentInteraction(interaction: MessageComponentInteraction) {
        ...
        const embedBuilder = EmbedBuilder.from(embed).setDescription(split.join("\n"));
    ...
    async messageLevel(message: Message): Promise<any> {
        ...
        let data = NorthClient.storage.guilds[message.guildId].levelData.get(message.author.id);
    ...
    async setPresence() {
        this.client.user.setPresence({ activities: [{ name: `Important! Click!`, type: ActivityType.Playing }], status: "dnd", afk: true });
    }
    ...
    async readServers() {
        const results = await query("SELECT * FROM configs WHERE id <> '622311594654695434'");
    ...
    async readGiveaways() {
        const results = await query("SELECT * FROM giveaways WHERE guild <> '622311594654695434' ORDER BY endAt ASC");
        console.log(`[${this.client.id}] ` + "Found " + results.length + " giveaways");
        results.forEach(async (result: any) => {
            const currentDate = Date.now();
            const millisec = result.endAt - currentDate;
            try {
    ...
    async readPoll() {
        ...
        results.forEach(async (result: any) => {
            const currentDate = Date.now();
            const time = new Date(result.endAt).getTime() - currentDate;
            try {
                const channel = <TextChannel>await this.client.channels.fetch(result.channel);
    ...
    async readNoLog() {
        const results = await query("SELECT id FROM users WHERE no_log = 1");
    ...
    async guildMemberAdd(member: GuildMember) {
        ...
        try {
            ...
            if (!channel || !channel.permissionsFor(guild.members.me).has(BigInt(18432))) return;
            ...
            if (welcome.image) {
                ...
                img.onload = async () => {
                    ...
                    const attachment = new AttachmentBuilder(canvas.toBuffer()).setName("welcome-image.png");
                    try {
    ...
    async guildMemberRemove(member: GuildMember | PartialGuildMember) {
        ...
        try {
            ...
            if (guild.members.me.permissions.has(BigInt(128))) {
    ...
    async messageReactionAdd(r: MessageReaction | PartialMessageReaction, user: User | PartialUser) {
        const roleMessage = NorthClient.storage.rm.find(x => x.id == r.message.id);
    ...
    async messageDelete(message: Message | PartialMessage): Promise<any> {
        const roleMessage = NorthClient.storage.rm.find(x => x.id === message.id);
        if (!roleMessage) return;
    ...
    async inviteCreate(invite: Invite) {
        let invites = NorthClient.storage.guilds[invite.guild.id]?.invites;
...
export class AliceHandler extends Handler {
    ...
    async setPresence() {
        this.client.user.setActivity("Aincrad", { type: ActivityType.Listening });
    }
    ...
    async preRead() {
        ...
        res.forEach(async result => {
            const endAfter = result.endAt.getTime() - Date.now();
            const mc = await profile(result.mc);
            let username = "undefined";
            if (mc) username = mc.name;
            const dc = `<@${result.user}>`;
            const rank = result.dc_rank;
            const title = `${dc} - ${rank} [${username}]`;
            setTimeout_(async () => {
                const asuna = await this.client.users.fetch("461516729047318529");
                try {
                    ...
                    try {
                        ...
                        const user = await this.client.users.fetch(result.user);
        ...
        setInterval(async () => {
            try {
                ...
                for (const result of NorthClient.storage.gtimers) {
                    ...
                    let dc = "0";
                    try {
                        const user = await this.client.users.fetch(str);
                        dc = user.id;
                    } catch (err: any) { }
                    const rank = result.dc_rank;
                    const title = `<@${dc}> - ${rank} [${username}]`;
                ...
                if (tmp.length <= 10) {
                    ...
                    const msg = await timerMsg.edit({ content: null, embeds: [allEmbeds[0]] });
                    let s = 0;
                    await msg.react("⏮");
                    await msg.react("◀");
                    await msg.react("▶");
    ...
    async preWelcomeImage(channel: TextChannel) {
        await channel.send({ files: [new AttachmentBuilder("https://cdn.discordapp.com/attachments/714804870078660630/998803804344569876/standard_6.gif")] });
    }
    ...
    async message(message: Message) {
        if (message.channel.id == "647630951169523762") {
            ...
            try {
                ...
                let res;
                try {
                    const f = await fetch(`https://api.slothpixel.members.me/api/players/${mcUuid}?key=${process.env.API}`);
                ...
                const results = await query(`SELECT id FROM dcmc WHERE dcid = '${dcUserID}'`);
                if (results.length == 0) {
            ...
            for (const lang in this.langMap) {
                if (message.channelId == this.langMap[lang]) {
                    if (lang === "english" && message.member.permissions.has(BigInt(32))) {
                        const msg = await message.channel.send({ content: "Do you want to accept translation for this message?", components: [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder({ customId: "yes", label: "Yes", emoji: "✅", style: ButtonStyle.Success }), new ButtonBuilder({ customId: "no", label: "No", emoji: "✖️", style: ButtonStyle.Danger }))] });
                    ...
                    const allRows: ActionRowBuilder<SelectMenuBuilder | ButtonBuilder>[][] = [];
                    ...
                    for (let ii = 0; ii < pages; ii++) {
                        ...
                        allEmbeds.push(em);
                        allRows.push([new ActionRowBuilder<SelectMenuBuilder>().addComponents(menu), new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder({ customId: "previous", emoji: "◀️", style: ButtonStyle.Primary }), new ButtonBuilder({ customId: "search", label: "Search by ID", emoji: "🔍", style: ButtonStyle.Secondary }), new ButtonBuilder({ customId: "next", emoji: "▶️", style: ButtonStyle.Primary }))]);
                    }
                    var s = 0;
                        ...
                        if (interaction.isButton()) {
                            switch (interaction.customId) {
                                ...
                                case "search":
                                    ...
                                    if (!trans) {
                                        await received.editReply({ embeds: [], components: [], content: `The message with ID ${id} doesn't exist!` });
                                        collector.emit("end");
                                        return;
                                    }
                                    ...
                                    await received.editReply({ embeds: [], components: [], content: `Linked translation to message ${id}.` });
                                    await message.react("✅");
                                    collector.emit("end");
                            }
