...
export default class EnergyDrinkItem implements Item {
    ...
    name = "EnergyDrink"
    async run(message: NorthMessage | NorthInteraction, msg: Message, em: EmbedBuilder, itemObject: any) {
        let newDateSql = jsDate2Mysql(new Date(Date.now() + 86400000));
        const author = message instanceof Message ? message.author.id : message.user.id;
        try {
            const results = await query(`SELECT doubling FROM users WHERE id = '${author}'`);
