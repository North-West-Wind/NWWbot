...
class AnnounceCommand implements FullCommand {
  ...
  async execute(interaction: NorthInteraction) {
    ...
    if (!channel.permissionsFor(interaction.guild.members.me).has(BigInt(2048))) return await interaction.reply(genPermMsg(2048, 1));
  ...
  async run(message, args) {
    const channel = await message.guild.channels.resolve(args[0].replace(/<#/g, "").replace(/>/g, ""))
    if (!channel || channel == undefined || channel == null) return await message.channel.send("The channel is not valid!");
    if (!channel.permissionsFor(message.guild.members.me).has(BigInt(2048))) return await message.channel.send(genPermMsg(2048, 1));
