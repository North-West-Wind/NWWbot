...
async function findAllSearchResult(term) {
  ...
  const items = Array.from(searchItem).filter((item: any) => item?.attribs?.href?.startsWith("/memes/"));
  return Array.from(new Set(items.map((item: any) => config.BASE_URL + item.attribs.href)));
}
...
class KnowYourMemeCommand implements FullCommand {
  ...
  async getEmbeds(keywords) {
    const results = await doSearch(keywords);
    const allEmbeds = [];
    let num = 0;
