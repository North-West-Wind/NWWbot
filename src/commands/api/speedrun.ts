...
class SpeedrunCommand implements SlashCommand {
    ...
    async execute(interaction: NorthInteraction) {
        ...
        const gameFetch = <any> await fetch(`https://www.speedrun.com/api/v1/games/${escape(game)}`).then(res => res.json());
    ...
    async run(message: NorthMessage, args: string[]) {
        const gameFetch = <any> await fetch(`https://www.speedrun.com/api/v1/games/${escape(args.join(" "))}`).then(res => res.json());
    ...
    async chooseGame(message: Discord.Message | NorthInteraction, name: string) {
        ...
        var result = <any> await fetch(`https://www.speedrun.com/api/v1/games?name=${escape(name)}&_bulk=1`).then(res => res.json());
    ...
    async getEmbedsByID(data) {
        const allEmbeds = [];
        const results = <any> await fetch(`https://www.speedrun.com/api/v1/games/${data.id}/records`).then(res => res.json());
        for (const record of results.data) {
            if (record.level) var levelFetch = <any> await fetch(`https://www.speedrun.com/api/v1/levels/${record.level}`).then(res => res.json());
            if (record.category) var categoryFetch = <any> await fetch(`https://www.speedrun.com/api/v1/categories/${record.category}`).then(res => res.json());
            ...
            for (const run of record.runs) {
                if (run.run.system.platform) var platformFetch = <any> await fetch(`https://www.speedrun.com/api/v1/platforms/${run.run.system.platform}`).then(res => res.json());
                if (run.run.system.region) var regionFetch = <any> await fetch(`https://www.speedrun.com/api/v1/regions/${run.run.system.region}`).then(res => res.json());
                ...
                if (run.run.players[0].rel === "guest") player = run.run.players[0].name;
                    ...
                    const userFetch = <any> await fetch(run.run.players[0].uri).then(res => res.json());
