...
const { twoDigits, setTimeout_, ms, altGetData } = require("../function.js");
...
module.exports = {
  ...
  async execute(message, args) {
    if (!args[0]) {
      return message.channel.send(
        `Proper usage: ${message.prefix}${this.name} ${this.usage
  ...
  async create(message) {
    ...
    setTimeout_(async function () {
      if (msg.deleted === true) {
        altGetData("DELETE FROM poll WHERE id = " + msg.id, function (
          err,
          result
        ) {
          if (err) {
        ...
        altGetData("SELECT * FROM poll WHERE id = " + msg.id, async function (
          err,
          results
        ) {
          if (err) {
  ...
  async end(message, args) {
    ...
    altGetData("SELECT * FROM poll WHERE id = '" + msgID + "'", async function (
      err,
      result
    ) {
      if (err) {
      ...
      try {
        ...
        altGetData("DELETE FROM poll WHERE id = " + result[0].id, function (
          err
        ) {
          if (err) {
            console.error(err);
