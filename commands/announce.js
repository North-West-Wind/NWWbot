module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!channel.permissionsFor(message.guild.me).has(this.permission)) return await message.channel.send(console.genPermMsg(this.permission, 1));
    if (!channel.permissionsFor(message.member).has(this.permission)) return await message.channel.send(console.genPermMsg(this.permission, 0));
