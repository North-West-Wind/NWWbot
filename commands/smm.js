...
module.exports = {
  ...
  async execute(message, args) {
    if(!args[0]) return message.channel.send("Please provide a query for searching!")
    smm.searchCourses({
    title: args.join(" ")
}, async(error, courses) => {
    ...
    for(var i = 0; i < courses.length; i++) {
      ...
      if(courses[i].description == undefined) var description = "No description.";
      ...
      async function edit(msg, s) {
                                msg.edit(allEmbeds[s]).then(async msg => {
                                  try {
                                    await msg.react("⏮");
                                    await msg.react("◀");
                                    await msg.react("▶");
