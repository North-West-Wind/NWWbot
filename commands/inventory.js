...
module.exports = {
  ...
  async execute(message, args, pool) {
    pool.getConnection(function(err, con) {
      ...
      con.query("SELECT * FROM inventory WHERE id = '" + message.author.id + "'", function(err, result) {
        if(err) return message.reply("there was an error trying to execute that command!");
        var itemObject;
        if(result.length == 0) {
          itemObject = {
        ...
        con.query("SELECT * FROM shop", async function(err, IResult) {
          ...
          for(var i = 0; i < IResult.length; i++) {
            item.push(`**${IResult[i].id}.** ${IResult[i].name} - **${itemObject[IResult[i].id.toString()]}**`)
          }
          ...
          var collected = await message.channel.awaitMessages(x => x.author.id === message.author.id, { max: 1, time: 30000, errors: ["time"]}).catch(console.error);
          ...
          em.setTitle(wanted.name).setDescription(wanted.description + `\nQuantity: **${itemObject[wanted.id.toString()]}**\n\n1️⃣ Use\n2️⃣ Return`).setFooter("Use item?", message.client.user.displayAvatarURL());
          msg.edit(em);
          await msg.react("1️⃣");
          await msg.react("2️⃣");
          
          var collected2 = await msg.awaitReactions((reaction, user) => ["1️⃣", "2️⃣"].includes(reaction.emoji.name) && user.id === message.author.id, { max: 1, time: 30000, errors: ["time"]}).catch(console.error);
          ...
          if(r.emoji.name === "1️⃣") {
            ...
            const itemFiles = fs.readdirSync(__dirname + "/../items").filter(file => file.endsWith(".js"));
