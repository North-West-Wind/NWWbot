...
const { twoDigits, setTimeout_, jsDate2Mysql, readableDateTime, genPermMsg, ms, readableDateTimeText } = require("../function.js");
  ...
  async execute(message, args) {
    ...
    if (args[0] === "create") {
      if (args[1]) {
        ...
        message.channel.send(`Created new giveaway in channel <#${channel.id}> for**${readableDateTimeText(time)}** with the item **${item}** and **${winnerCount} winner${winnerCount > 1 ? "s" : ""}**.`);
        var reacted = [];
        var currentDate = new Date();

        var newDate = new Date(currentDate.getTime() + time);
        var newDateSql = jsDate2Mysql(newDate);
        var readableTime = readableDateTime(newDate);
