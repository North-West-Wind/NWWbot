...
import { ApplicationCommand, ApplicationCommandOption, ApplicationCommandOptionType, Interaction, InteractionResponse } from "../../classes/Slash";
...
const config = {
  ...
  async function doSearch(term) {
    ...
    for (const result of resultUrls) {
      let body;
      try { body = await makeRequest(result);
      } catch (e) { throw e; }
...
export class KnowYourMeme implements SlashCommand {
    name: "knowyourmeme";
    ...
    async execute(message: NorthMessage, ...args: string[]) {
