...
import { ApplicationCommand, ApplicationCommandOption, ApplicationCommandOptionChoice, ApplicationCommandOptionType, FakeMessage, Interaction, InteractionResponse } from "../../classes/Slash";
...
export class AkiCommand implements SlashCommand {
    ...
    async postSlash(client: NorthClient, interaction: Interaction, args: any[]) {
        ...
        args = args?.map(x => <string>x?.value).filter(x => !!x) || [];
        await this.execute(await InteractionResponse.createFakeMessage(client, interaction), ...<string[]>args);
      }
    ...
    async region(message: NorthMessage | FakeMessage) {
