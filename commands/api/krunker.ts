...
class KrunkerCommand implements SlashCommand {
    ...
    async execute(obj: { interaction: Interaction, args: any[] }) {
        if (obj.args[0].name === "server") {
            ...
            await this.server(msg, obj.args[0]?.options[0]?.value || null, <NorthClient> obj.interaction.client, msg.author);
    ...
    async server(msg: Discord.Message, search: string, client: NorthClient, author: Discord.User) {
        try {
