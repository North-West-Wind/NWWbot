...
class LyricsCommand implements SlashCommand {
    ...
    async execute(obj: { interaction: Interaction, args: any[] }) {
        await obj.interaction.reply("Lyrics are loading!");
        ...
        const message = await obj.interaction.fetchReply();
        if (!author && !title) {
            await message.channel.send("Cannot find the song! Try to be more specific?");
            return;
        }
    ...
    async createLyricsEmbeds(lyrics: string, title: string, author: string, icon: string, client: Discord.Client) {
