...
import { ApplicationCommand, ApplicationCommandOption, ApplicationCommandOptionType, ApplicationCommandOptionChoice, InteractionResponse, Interaction, FakeMessage } from "../../classes/Slash";
import * as Discord from "discord.js";
import { NorthClient } from "../../classes/NorthClient";
import { color, readableDateTime, numberWithCommas, nameToUuid, createEmbedScrolling } from "../../function.js";
import { NorthMessage } from "../../classes/NorthMessage";

export class HypixelCommand implements SlashCommand {
    name: "hypixel";
    description:
    ...
    async postSlash(client: NorthClient, interaction: Interaction, args: any[]) {
  ...
  async execute(message: NorthMessage | FakeMessage, ...args: string[]) {
    ...
    if (args[0] === "auctionhouse" || args[0] === "ah") {
      ...
      const items = <Discord.Collection<string, any>>new Discord.Collection(Object.entries(itemIDs));
    ...
    if (args[0] === "guild" || args[0] === "g") {
      ...
      body.player.achievementsOneTime.forEach(item => message.author.send(item.replace(/_/g, " ").charAt(0).toUpperCase() + item.substring(1)));
    } else if (args[0] === "tnt") {
