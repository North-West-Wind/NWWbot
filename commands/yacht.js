...
module.exports = {
    ...
    async execute(message) {
        ...
        const em = new Discord.MessageEmbed()
            ...
            .setTimestamp()
            .setFooter("Please type in commands.", message.client.user.displayAvatarURL());
        var msg;
        async function end() {
        ...
        async function loop(i) {
            ...
            em.setTitle(`Yacht Dice Game (Round ${i + 1})`).setDescription(str + `\nCommands:\n**Roll** - Roll the dices (${3 - rolled} times left)\n**Lock <index>** - Lock the dices with indexes 1 to 6\n**Score <category>** Choose a category to place your score and move to the next turn\n**End** - End the game immediately`);
            try {
              if (!msg) msg = await message.channel.send(em);
              else msg = await msg.edit(em);
            } catch(err) { return; }
            const collector = message.channel.createMessageCollector(m => m.author.id === message.author.id, { max: Infinity, idle: 60000 });
                    ...
                    case "score":
                        ...
                        switch (args.join(" ").toLowerCase()) {
                            ...
                            case "3 of a kind":
                                ...
                                for (let o = 0; o < 6; o++) {
                                    const filtered = dices.filter(x => x.number === o);
