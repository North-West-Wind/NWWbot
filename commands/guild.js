...
const { ms, createEmbedScrolling, altGetData } = require("../function.js");
		...
		switch (args[1]) {
			case "create":
				...
				setTimeout_(async () => {
					...
					altGetData(`SELECT id FROM gtimer WHERE user = '${user.id}' AND mc = '${uuid[0].id}' AND dc_rank = '${escape(args.slice(4).join(" "))}'`, (err, results) => {
						if (err) return asuna.send(title + " expired");
						if (results.length == 0) return;
						asuna.send(title + " expired");
						altGetData(`DELETE FROM gtimer WHERE user = '${user.id}' AND mc = '${uuid[0].id}' AND dc_rank = '${escape(args.slice(4).join(" "))}'`, (err) => {
			...
			case "delete":
				...
				altGetData(`SELECT * FROM gtimer WHERE user = '${userd.id}'`, (err, results) => {
					...
					altGetData(`DELETE FROM gtimer WHERE user = '${userd.id}'`, (err) => {
